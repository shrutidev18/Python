...
class RPSGUI:
    ...
    def __init__(self, root):
        ...
        self.rock_img = tk.PhotoImage(file="rock.png").subsample(4, 4)  # Adjust numbers as needed
        self.paper_img = tk.PhotoImage(file="paper.png").subsample(4, 4)
        self.scissors_img = tk.PhotoImage(file="scissors.png").subsample(8, 8)
    ...
    def build_gui(self):
        ...
        #style dropdown
        style = ttk.Style()
        style.theme_use("default")  # or try 'clam', 'alt', 'vista', 'xpnative'
        ...
        style.map(
            "CustomCombobox.TCombobox",
            fieldbackground=[("readonly", "lightgray")],
        ...
        #self.lang_menu = ttk.Combobox(lang_frame, values=list(self.languages.values()))
        self.lang_menu = ttk.Combobox( lang_frame,values=list(languages.values()),width=20,style="CustomCombobox.TCombobox")
        ...
        tk.Button(btn_frame, image=self.rock_img, command=lambda: self.play("R")).grid(row=0, column=0, padx=10)
        tk.Button(btn_frame, image=self.paper_img, command=lambda: self.play("P")).grid(row=0, column=1, padx=10)
        tk.Button(btn_frame, image=self.scissors_img, command=lambda: self.play("S")).grid(row=0, column=2, padx=10)
    ...
    def toggle_theme(self):
        if self.theme == "light":
            ...
            self.title_label.configure(bg="#2E2E2E", fg="white")
            self.move_label.configure(bg="#2E2E2E", fg="white")
            self.result_label.configure(bg="#2E2E2E", fg="white")
            self.score_label.configure(bg="#2E2E2E", fg="white")
            ...
            self.move_label.configure(bg="SystemButtonFace", fg="black")
            self.result_label.configure(bg="SystemButtonFace", fg="black")
            self.score_label.configure(bg="SystemButtonFace", fg="black")
            self.toggle_btn.config(
                text="🌙 Switch to Dark Mode",
                bg="#ffefd5",
                fg="#333",
    ...
    def set_language(self, event):
        ...
        self.lang = next(code for code, name in self.languages.items() if name == selected_name)
        self.refresh_labels()
    ...
    def play(self, player_move):
        ...
        comp_text = f"Computer chose: {move_dict[comp_move]}"
    ...
    text=f"{comp_text}\n{outcome} (Round {self.current_round}/{self.total_rounds})"
)
